...
@if(!$sudahIsiHariIni)
    <div class="status-warning bg-amber-50/75 border border-amber-200/80 text-amber-800 px-4 py-3 mb-4 text-sm flex items-center gap-2">
        ⏰ Kamu belum isi jurnal hari ini. <a href="{{ route('kegiatan.create') }}" class="font-semibold underline">Isi sekarang</a>
    </div>
@else
    <div class="status-success bg-emerald-50/75 border border-emerald-200/80 text-emerald-800 px-4 py-3 mb-4 text-sm">
...
<div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
    ...
    <div class="glass-card p-5 flex flex-col justify-center gap-2">
        <a href="{{ route('kegiatan.create') }}" class="btn-primary text-white text-center py-2 rounded-lg text-sm font-medium">
            + Isi Jurnal Hari Ini
        </a>
        <a href="{{ route('kegiatan.index') }}" class="btn-secondary text-gray-700 text-center py-2 rounded-lg text-sm font-medium">
            Lihat Semua Riwayat
        </a>
    </div>
</div>
...
<div class="glass-card overflow-hidden">
    <div class="px-5 py-4 border-b border-white/70">
        <h2 class="font-semibold text-gray-800">Kegiatan Terbaru</h2>
    </div>

    @forelse($kegiatanTerakhir as $k)
        <div class="px-5 py-4 border-b border-white/70 last:border-0">
